...
enum class clock_type : uint32_t { REALTIME = 0, MONOTONIC = 1, BOOTTIME = 7 };
...
enum class rlimit_resource : int { NOFILE = 7 };
...
enum class prctl_num { SET_NAME = 15 };
...
void assert2_impl(const char *file, int line, const char *function,
                  const char *msg);
...
void *mmap(void *addr, size_t length, int prot, bionic::mmap_flags flags,
...
int prctl(bionic::prctl_num opt, unsigned long a2, unsigned long a3,
...
template <class T> struct tm_detail {
  tm loc;
  using source = T;
...
template <> struct arg_rewrite<tm *> : tm_detail<tm *> {};
template <> struct arg_rewrite<const tm *> : tm_detail<const tm *> {};
